...
MODEL_CONTEXT_LIMITS = {
    ...
    "deepseek-V2-chat": 32768,
}

# Define API endpoints
OMNIPLEX_API_URL = "https://omniplex.ai/api/chat"
FFA_API_URL = "https://ffa.chat/api/chat/completions"
...
"""
        ...
        if self.model in ["claude-3-5-sonnet-20240620", "gemini-1.5-pro", "DeepSeek-Coder-V2", "deepseek-V2-chat"]:
            return self.send_message_ffa(chat_history, message)
        else:
            return self.send_message_omniplex(chat_history, message)

    def send_message_omniplex(self, chat_history, message):
        payload = {
        ...
        typing_indicator = " |"
        with st.spinner(""):
            response = requests.post(OMNIPLEX_API_URL, headers={'Content-Type': 'application/json'}, json=payload, stream=True)
    ...
    def send_message_ffa(self, chat_history, message):
        headers = {
            ...
            'Cookie': 'Hm_lvt_e1ab0b76810746f50a7929d521eee278=1721296012; Hm_lpvt_e1ab0b76810746f50a7929d521eee278=1721296012; HMACCOUNT=A63480C9E06402CB; token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImVlN2JlMjg2LWQyNWEtNGI5Yy1hZTlhLTVjNTVjNzZiNTcwYSJ9.-vWeEQiW1m2E1K8SyDqgxeBfjomLuUfFX8Hz2bnJrq0'
        }
        payload = {
            "model": self.model,
            "stream": True,
            "messages": chat_history + [{"role": "user", "content": message}],
            "chat_id": "d3ba18f6-4b98-4f49-a1ea-ac7258dd2b4d"  # You might want to generate this dynamically
        }
        response_text = ""
        typing_indicator = " |"
        with st.spinner(""):
            response = requests.post(FFA_API_URL, headers=headers, json=payload, stream=True)
        if response.status_code == 200:
            for line in response.iter_lines():
                if line:
                    chunk_str = line.decode('utf-8').replace('data: ', '')
                    if chunk_str != '[DONE]':
                        try:
...
def count_tokens(message, model):
    """
    Count the number of tokens in a message for a given model.
    For non-tiktoken models, return a default value to bypass the check.
    
    Args:
    message (str): The input message to tokenize.
    model (str): The name of the model to use for tokenization.
    
    Returns:
    int: The number of tokens in the message, or a default value for non-tiktoken models.
    """
    try:
        if model in ["gpt-4o", "gpt-4-turbo-2024-04-09", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo"]:
            encoding = tiktoken.encoding_for_model(model)
            return len(encoding.encode(message, disallowed_special=()))
        else:
            # For non-tiktoken models, return a default value
            # This effectively bypasses the token limit check
            return 1  # Or any other small number
    except Exception as e:
        st.toast(f"Tokenization error: {str(e)}", icon="❌")
        return 1  # Return a small number to allow processing the message
...
def main_ui():
    if st.session_state.username is None:
        ...
        if selected_model in ["gpt-4o", "gpt-4-turbo-2024-04-09", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo"]:
            user_input_tokens = count_tokens(user_input, selected_model)
